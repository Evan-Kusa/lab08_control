...
Publishes Hover commands directly to the Crazyflie server, bypassing vel_mux.
Supports loading per-student PID gains from a YAML file.

Subscribes: /goal_pose (PoseStamped)
Publishes:  /crazyflie_real/cmd_hover (Hover)
Uses TF:    crazyflie_real/odom -> crazyflie_real (configurable)
"""
...
class GoalController(Node):
    def __init__(self):
        ...
        self.declare_parameter('tf_world_frame', 'crazyflie_real/odom')
        self.declare_parameter('tf_robot_frame', 'crazyflie_real')
        self.declare_parameter('control_rate', 10.0)
        self.declare_parameter('position_tolerance', 0.05)
        self.declare_parameter('yaw_tolerance', 0.01)
        self.declare_parameter('cmd_topic', '/crazyflie_real/cmd_hover')
        ...
        # PID gains - Z axis (not used with Hover z_distance, but kept for completeness)
        ...
        if student_id > 0:
            if not gains_file:
                gains_file = os.path.join(
                    os.path.dirname(__file__), 'student_gains.yaml')
            try:
                with open(gains_file, 'r') as f:
                    all_gains = yaml.safe_load(f)
                sg = all_gains['students'][student_id]
                for axis in ('pid_x', 'pid_y'):
                    for field in ('kp', 'ki', 'kd'):
                        if field in sg[axis]:
                            self.set_parameters([rclpy.parameter.Parameter(
                                ...
                                float(sg[axis][field]))])
                self.get_logger().info(f"Loaded gains for student {student_id}")
            except Exception as e:
                self.get_logger().error(f"Failed to load student {student_id} gains: {e}")
        ...
        self.cmd_pub = self.create_publisher(Hover, cmd_topic, 10)
        ...
        self.get_logger().info(
            ...
            f"cmd: {cmd_topic}, rate: {self.control_rate} Hz"
        )
        self._log_gains()
    ...
    def control_loop(self):
        ...
        dx = gx - x
        dy = gy - y
        distance = math.sqrt(dx**2 + dy**2 + (gz - z)**2)
        ...
        cmd = Hover()
        cmd.vx = float(vx_body)
        cmd.vy = float(vy_body)
        cmd.yaw_rate = float(-wz)
        cmd.z_distance = float(gz)
        self.cmd_pub.publish(cmd)
